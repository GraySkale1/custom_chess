...
def piece_add(sprites_list:list):
    sprites_list = [sprites_list[0]]
...
def on_mouse_press(x, y, button, modifiers):
    ...
    if increment == 0:
        ...
        if highlight in sprites:
            sprites.remove(highlight)
        sprites = piece_add(sprites)
    else:
       highlight.position = (index_to_pixel([p1,p2])[0], index_to_pixel([p1,p2])[1], highlight.z)
       sprites.insert(1, highlight)
